...
<ComponentStackContainer>
  <div class="relative" use:scrollRegistrar>
    <HeroCarousel
      class="h-[calc(100vh-7.5rem)] relative"
        ...
        class={classNames(
          'flex flex-col pt-16 pb-8 px-32 transition-opacity inset-x-0 bottom-0 absolute delay-150',
          {
            'opacity-0 pointer-events-none':
              !$topVisible && $selectedTmdbEpisode,
          },
        )}
        style="transform: translateZ(0);"
      >
        ...
        >
          ...
          {#if trailerId}
            <Button
              on:clickOrSelect={() =>
                trailerId &&
                background?.playYoutubeVideo({ tmdbId, videoId: trailerId })}
            >
          ...
          {#if !$inLibrary}
            <Button action={handleAddToLibrary} icon={Bookmark}
              >Add to Library</Button
            >
          {:else}
            <Button action={handleRemoveFromLibrary} icon={Minus}
    ...
    <div class="relative z-10" style={$visibleStyle}>
            ...
            {#each $episodes as episode, i (episode.id)}
              ...
              {#key episode.id}
                <TmdbEpisodeCard
                  ...
                  on:clickOrSelect={() =>
                    (sheetProps = { episode, series: tmdbSeries })}
                />
              {/key}
            {/each}
      ...
      {#await $tmdbSeries then series}
        <Carousel
          scrollClass="px-32"
          class="mb-8"
          on:enter={scrollIntoView({ top: 64 + 32 })}
        >
          <div slot="header">
        ...
        >
          ...
          <div class="text-zinc-300 font-medium text-lg flex flex-wrap">
            <div class="flex-1">
              <div class="mb-8">
                <h2
                  class="uppercase text-sm font-semibold text-zinc-500 mb-0.5"
                >
                  Created By
                </h2>
